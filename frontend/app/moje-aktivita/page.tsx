...
export default async function MyActivityPage() {
  ...
  const [
    ...
    supabase
      ...
      .select("*, subject:subject_id(name, slug, short_tag)")
        ...
        select: (columns: string) => {
          eq: (column: string, value: string) => {
            order: (column: string, options: { ascending: boolean }) => Promise<{
              data: SubjectProposalRecord[] | null;
            }>;
          };
        };
      };
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {deck.card_count} karet · {deck.is_public ? "Veřejný" : "Soukromý"}
                  </p>
                  {deck.subject && (
                ...
                <StatusBadge tone={deck.is_public ? "success" : "muted"}>
            ...
            <div key={material.id} className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {material.subject && (
                    <Link href={`/predmety/${material.subject.slug}`} className="mt-1 block text-sm text-muted-foreground hover:text-foreground">
                  ...
                  {material.rejection_reason && (
                    <p className="mt-2 text-sm text-destructive">
                      Důvod zamítnutí: {material.rejection_reason}
                    </p>
                  )}
                </div>
                <StatusBadge tone={material.moderation_status === "approved" ? "success" : material.moderation_status === "rejected" ? "danger" : "warning"}>
                  {getMaterialStatusLabel(material.moderation_status)}
                </StatusBadge>
              </div>
            </div>
          ))}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-foreground">
                      {proposalData.name || proposalData.short_tag || "Návrh předmětu"}
                    </p>
                    <p className="mt-1 text-sm text-muted-foreground">
                      {proposal.type === "new" ? "Nový předmět" : "Úprava existujícího"} · {new Date(proposal.created_at).toLocaleDateString("cs-CZ")}
                    </p>
                    {proposal.rejection_reason && (
                      <p className="mt-2 text-sm text-destructive">
                        Důvod zamítnutí: {proposal.rejection_reason}
                      </p>
                    )}
                  </div>
                  <StatusBadge tone={proposal.status === "approved" ? "success" : proposal.status === "rejected" ? "danger" : "warning"}>
          ...
          {myFeedback.map((item) => (
            <div key={item.id} className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <StatusBadge tone={item.status === "resolved" ? "success" : item.status === "in_progress" ? "warning" : "muted"}>
