...
export function SessionSummary({ results, subjectSlug, onRestart }: SessionSummaryProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full max-w-lg mx-auto px-4 ">
      ...
      {/* Stats */}
      <div className="w-full surface-card p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full">
        <button
          onClick={onRestart}
          className="flex-1 px-5 py-3 rounded-xl text-sm font-medium primary-action text-white hover:opacity-90 transition-colors text-center"
        >
           Procvičovat znovu
        </button>
        {subjectSlug && (
          <Link
            ...
            className="flex-1 px-5 py-3 rounded-xl text-sm font-medium border border-border bg-card hover:bg-muted transition-colors text-center"
          >
            ← Zpět na předmět
          </Link>
        )}
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1">
        ...
        <div className="h-2 bg-muted rounded-md overflow-hidden">
          <div
            className={`h-full ${color} rounded-md transition-colors duration-700`}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
