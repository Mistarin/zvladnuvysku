ALTER TABLE public.subject_materials
  ADD COLUMN IF NOT EXISTS points_override smallint
  CHECK (points_override IN (0, 2, 3));
...
AS $function$
  ...
  totals AS (
    SELECT
      ...
      (fs.flashcard_points + ms.material_points + ss.subject_points) AS approved_score
