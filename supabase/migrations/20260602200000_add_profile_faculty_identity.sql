...
ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_faculty_check;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_faculty_check
  CHECK (
    faculty IS NULL
    OR faculty IN ('FSS', 'FU', 'FF', 'LF', 'PdF', 'PřF')
  );
...
AS $function$
  ...
  combined AS (
    SELECT
      COALESCE(f.user_id, m.user_id, s.user_id, t.user_id) AS user_id,
  ...
  SELECT
    ...
    p.display_name,
    p.faculty,
    (c.flashcard_points + c.material_points + c.subject_points + c.teacher_points) AS total_score,
    c.flashcard_count,
    c.material_count,
    c.subject_count,
    c.teacher_count
  ...
  WHERE p.display_name IS NOT NULL
    ...
    AND (c.flashcard_points + c.material_points + c.subject_points + c.teacher_points) > 0
  ORDER BY
    total_score        DESC,
...
AS $function$
  ...
  totals AS (
    SELECT
      ...
      (fs.flashcard_points + ms.material_points + ss.subject_points + ts.teacher_points) AS approved_score
...
DROP FUNCTION IF EXISTS public.get_public_profile_summaries(uuid[]);
